#!/bin/python
# train, test, evaluate a random forest to predict enhancer activity 
...
# =============  main =============
rdf = pd.read_csv(os.path.join(ROOT_PATH,RAW_FILE), sep ='\t',header=None,index_col=None)

### TIDY UP DATA
rdf.columns = ["TE_chr", "TE_start", "TE_end", "TE", "TE_family", "TF_chr", "TF_start", "TF_end", "TF", "TE_info", "enhc_chr", "enhc_start", "enhc_end", "enhc_overlap"]
...
mdf = rdf.groupby(['TE_id']).mean()[['enhc_overlap']]
X = df.values
Y = mdf.values
